        ...
        return values?.map((value: number | Animated.Value, index: number) => {
            ...
            if (value instanceof Animated.Value) {
                if (valueToSet instanceof Animated.Value) {
                    valueToSet = (valueToSet as any).__getValue() as number;
                }
...
export class Slider extends PureComponent<SliderProps, SliderState> {
            ...
            values: updateValues({
                values: normalizeValue(
                    ...
                    this.props.value instanceof Animated.Value
                        ? (this.props.value as any).__getValue()
                        : this.props.value,
                ),
            }),
        };
    ...
    componentDidUpdate() {
        ...
        newValues.forEach((value, i) => {
            ...
            } else if (value !== (this.state.values[i] as any).__getValue()) {
    ...
    _getRawValues(
        ...
        return values.map((value) => (value as any).__getValue());
    }
    ...
    _getCurrentValue = (thumbIndex = 0) =>
        (this.state.values[thumbIndex] as any).__getValue();
    ...
    _getThumbTouchRect = (thumbIndex = 0): RectReturn => {
        const {containerSize, thumbSize} = this.state;
        const {thumbTouchSize} = this.props;
